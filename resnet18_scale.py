...
wb = pd.DataFrame()
wb.to_excel("result_"+str(args.reduce_resolution)+".xlsx")
...
def train_model(model, dataloaders, criterion, optimizer, scheduler, num_epochs, is_inception=False,rst_dir='./rst/exp'):
    ...
    for epoch in range(num_epochs):
        ...
        #print(log)
        wb = load_workbook("result_" + str(args.reduce_resolution) + ".xlsx")
        wb1 = wb.active
        ...
        wb.save("result_" + str(args.reduce_resolution) + ".xlsx")
...
if __name__ == '__main__':
    """ Alexnet
            """
    preTrain=True
    #lr=[0.002]*5+[0.001]*10+[0.0005]*10+[0.0002]*5+[0.001]*5+[0.0005]*5+[0.0002]*5+[0.0001]*5
    lr=[0.001]*40+[0.0005]*10
    ...
    nofreeze_layers = ( "layer4", "fc")
    for name, param in model_ft.named_parameters():
        if name.split(".")[0] in nofreeze_layers:
